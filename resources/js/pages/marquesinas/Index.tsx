...
export default function Index({ auth, lista }: any) {
    ...
    const onSedes = (id: number) => {
        router.visit(`/marquesinas/${id}/sedes`);
    };
    ...
    const onTrash = async (id: number) => {
        ...
        if (result.isConfirmed) {
            router.delete(route('marquesinas.destroy', id), {
                preserveScroll: true,
                onSuccess: async () => {
                    ...
                    if (remainingItems === 0 && currentPage > 1) {
                        router.visit(`/marquesinas?page=${currentPage - 1}`);
                    } else {
                        router.visit(`/marquesinas?page=${currentPage}`);
                    }
                },
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="overflow-x-auto px-4">
                <Table
                    ...
                    titles={['Marquesina']}
                    actions={[
                        { icon: Building2, action: onSedes, title: 'Sedes' },
                ...
                <Modal show={show} closeable={true} title="Gestionar marquesinas">
                    <Form
                        id={id}
                        onReload={onReload}
                        onClose={() => {
