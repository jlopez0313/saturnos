...
type ThisForm = {
    marquesinas_id: string;
    departamentos_id: string;
    ciudades_id: string;
    sedes_id: string;
};

export const Form = ({ marquesina, onReload, onClose }: any) => {
    const { data, setData, post, processing, errors, reset } = useForm<Required<ThisForm>>({
        marquesinas_id: marquesina.id,
        departamentos_id: '',
        ciudades_id: '',
        sedes_id: '',
    });
    ...
    const submit: FormEventHandler = async (e) => {
        ...
        const options = {
            ...
            onError: (errors: any) => {
                console.log(errors);
                if (errors.marquesinas_id) {
                    reset('marquesinas_id');
                }
            },
        };

        post(route('marquesinas.store_sedes'), options);
    };

    useEffect(() => {
        const getData = async () => {
            try {
                const [{ data: {data: departamentos} }] = await axios.all([
