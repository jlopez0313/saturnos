...
export default function Index({ auth, lista }: any) {
    ...
    const onSedes = (id: number) => {
        router.visit(`/encuestas/${id}/sedes`);
    };
    ...
    const onTrash = async (id: number) => {
        ...
        if (result.isConfirmed) {
            router.delete(route('encuestas.destroy', id), {
                preserveScroll: true,
                onSuccess: async () => {
                    ...
                    if (remainingItems === 0 && currentPage > 1) {
                        router.visit(`/encuestas?page=${currentPage - 1}`);
                    } else {
                        router.visit(`/encuestas?page=${currentPage}`);
                    }
                },
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="overflow-x-auto px-4">
                ...
                <Modal show={show} closeable={true} title="Gestionar encuestas">
                    <Form
                        id={id}
                        onReload={onReload}
                        onClose={() => {
