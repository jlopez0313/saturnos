...
export default function Index({ auth, lista }: any) {
    ...
    const onTrash = async (id: number) => {
        ...
        if (result.isConfirmed) {
            router.delete(route('ventanillas.destroy', id), {
                preserveScroll: true,
                onSuccess: async () => {
                    ...
                    if (remainingItems === 0 && currentPage > 1) {
                        router.visit(`/ventanillas?page=${currentPage - 1}`);
                    } else {
                        router.visit(`/ventanillas?page=${currentPage}`);
                    }
                },
        ...
        const onSetData = () => {
            const data = lista.data.map((item: any) => {
                return {
                    ...
                    tiene_encuesta: item.tiene_encuesta == 1 ? 'SI' : 'NO',
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="overflow-x-auto px-4">
                <Table
                    user={auth.user}
                    data={data}
                    titles={['Ventanilla', 'Sede', 'Ciudad', 'Departamento', 'Tiene Encuesta', 'Estado']}
                    actions={[
                ...
                <Modal show={show} closeable={true} title="Gestionar ventanillas">
                    <Form
                        id={id}
                        onReload={onReload}
                        onClose={() => {
